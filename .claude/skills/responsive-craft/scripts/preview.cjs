...
const SERVE_SCRIPT = path.join(SCRIPTS_DIR, 'serve-static.cjs');
...
Usage:
  node preview.cjs <url>              Preview a running dev server
  node preview.cjs <path>             Preview a static HTML file
  node preview.cjs <url> --breakpoints 320,768,1920

Examples:
  node preview.cjs http://localhost:3000
  node preview.cjs ./index.html
  node preview.cjs http://localhost:5173 --breakpoints 375,768,1024,1440,1920
  `);
  process.exit(0);
}
