...
Usage:
  node snapshot.cjs <url>                          Capture at default breakpoints
  node snapshot.cjs <url> --before                 Save as baseline for comparison
  node snapshot.cjs <url> --breakpoints 320,768    Custom breakpoints
  node snapshot.cjs <url> --output ./my-shots      Custom output directory

Examples:
  node snapshot.cjs http://localhost:3000
  node snapshot.cjs http://localhost:3000 --before
  node snapshot.cjs http://localhost:3000 --breakpoints 375,768,1024,1440,1920
  `);
  process.exit(0);
}
